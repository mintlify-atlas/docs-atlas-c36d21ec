---
title: "Extend Page Agent with Custom Tools"
...
| `ask_user` | Ask the user a question and wait for their typed reply. |
| `click_element_by_index` | Click a DOM element identified by its index in the simplified page tree. |
| `input_text` | Type text into a focused input or textarea element. |
| `select_dropdown_option` | Choose an option in a `<select>` element. |
| `scroll` | Scroll the page or a scrollable container vertically. |
| `scroll_horizontally` | Scroll the page or a container horizontally. |
